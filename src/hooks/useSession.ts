...
const syncAuth = async (
  ...
  const travelToken = session.activeUid
    ? await authAPI.travel(session.activeUid)
    : null;
...
export const useSession = (
  ...
  const controls = (type: Controls, uid: string) => {
    ...
    if (type === "add" && parsedSession) {
      const updatedSession: Session = {
        ...parsedSession,
        activeUid: uid,
        members: !parsedSession.members.includes(uid)
          ? [...parsedSession.members, uid]
          : parsedSession.members,
      };
      ...
      const updatedSession: Session = {
        ...parsedSession,
        activeUid:
          parsedSession.activeUid && parsedSession.activeUid === uid
            ? null
            : uid,
        members: !parsedSession.members.includes(uid)
          ? [...parsedSession.members, uid]
  ...
  }, [parsedSession, auth, loading, user]);
